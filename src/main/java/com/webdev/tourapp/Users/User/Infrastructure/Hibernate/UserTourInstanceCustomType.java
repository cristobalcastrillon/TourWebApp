    ...
    public Object nullSafeGet(ResultSet rs, String[] names, SharedSessionContractImplementor session, Object owner) throws HibernateException, SQLException
    {
        ...
        try
        {
            ...
            if(value.isPresent())
            {
                ...
                response = objects.stream().map(element -> new CustomUUID ((String) element.get("tourInstanceID"))).collect(Collectors.toList());

            }
        }
        catch (Exception e) {
            throw new HibernateException("Error leyendo el mapa de UserTourInstanceIDs " + e.toString());
        }
        return Optional.ofNullable(response);
    }
    @Override
    public void nullSafeSet(PreparedStatement st, Object value, int index, SharedSessionContractImplementor session) throws HibernateException, SQLException {
        Optional <List<CustomUUID>> object = (value == null) ? Optional.empty() : (Optional<List<CustomUUID>>) value;
        try {
            if(object.isEmpty()) {
                ...
                List<HashMap<String, Object>> objects = object.get().stream().map(element -> element.dataDB("tourInstanceID")).collect(Collectors.toList());
